...
//! # Deprecation note
//!
//! [`Scheduler`](crate::Scheduler) is replaced by `TaskScheduler`. Implement
//! `TaskScheduler` and the blanket will satisfy `Scheduler` automatically.
//!
//! [`DurableStorage`](crate::DurableStorage) is replaced by `ExecutionStore +
//! StepStore + WaitGroupStore + EventStore`. Implement all four and the
//! blanket will satisfy `DurableStorage` automatically.
...
///
/// Covers the `zart_executions` and `zart_execution_runs` tables. Admin
/// business logic lives in `ExecutionService` (`zart` crate); this trait
/// exposes only the SQL primitives.
///
/// Replaces the execution-related methods of [`DurableStorage`](crate::DurableStorage).
#[async_trait]
pub trait ExecutionStore: Send + Sync {
    ...
    ///
    /// Creates a new run at `run_index + 1`, updates `current_run_id`, and
    /// returns the new `run_id`. Does NOT schedule a body task — the caller
    ...
    ///
    /// The `trigger` value is stored in `zart_execution_runs.trigger`
    /// (e.g. `'restart'` or `'selective_rerun'`). Business logic for choosing
    /// the trigger and computing which steps to rerun lives in `ExecutionService`.
...
///
/// Replaces the step-related methods of [`DurableStorage`](crate::DurableStorage).
#[async_trait]
...
///
/// Covers the wait-group columns of `zart_steps`.
///
/// Replaces the wait-group methods of [`DurableStorage`](crate::DurableStorage).
#[async_trait]
...
///
/// Replaces the event and stats methods of [`DurableStorage`](crate::DurableStorage).
#[async_trait]
