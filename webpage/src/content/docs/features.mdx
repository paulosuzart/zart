...
See [Parallel Steps](/rust-api/parallel-steps/) for full documentation.
...
See [Wait for Event](/rust-api/wait-for-event/) for full documentation.
...
Long-running steps — LLM agent calls, data pipelines, video processing — can take minutes or even hours. Without heartbeats, Zart's orphan recovery would detect these tasks as "stuck" and reset them back to `scheduled`, causing duplicate execution.
...
Heartbeat loop: every 100s → UPDATE locked_at = NOW()
    ↓ (handler runs for 20 minutes — LLM call, data pipeline, etc.)
