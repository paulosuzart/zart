...
Steps are defined with `#[zart_step]` or by manually implementing the [`ZartStep`](/docs/rust-api/zart-step/) trait. Each step's result is persisted to the database before the workflow moves on.
...
See [ZartStep](/docs/rust-api/zart-step/) for all available configuration options.
...
Returns `Err(TaskError::EventTimeout)` if the timeout elapses before the event arrives. See [Wait for Event](/docs/rust-api/wait-for-event/) for full patterns.
...
On first run the expression is evaluated and persisted. On replay the cached value is returned — the expression is never re-evaluated. See [Capture Variables](/docs/rust-api/capture-variables/) for full details and when to use captures vs regular steps.
...
See [Parallel Steps](/docs/rust-api/parallel-steps/) for error handling and dynamic fan-out patterns.
