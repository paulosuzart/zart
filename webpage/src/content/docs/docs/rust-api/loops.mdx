...
The [Durable Loops example](/docs/examples/durable-loops/) processes a batch of reports using both mechanisms: the `{index}` template for per-item processing steps and `.named()` for conditional notification steps.
