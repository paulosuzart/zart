...
<Aside type="tip">
In most applications the scheduler is shared via `Arc<PostgresStorage>` so both the web server (scheduling) and the worker (executing) share the same instance.
</Aside>
