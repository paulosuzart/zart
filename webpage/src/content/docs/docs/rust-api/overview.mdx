...
├────────────────────────────────────────────────────────────────┤
│  Scheduler Layer   TaskScheduler  DurableScheduler  Worker     │
...
| Type | Role |
|---|---|
| `TaskScheduler` | Trait (in `zart-scheduler`) — poll for due tasks, mark complete/failed |
| `PostgresTaskScheduler` | Concrete — PostgreSQL-backed task queue (`zart_tasks`) |
| `PostgresStorage` | Concrete — execution-side storage (`zart_executions`, `zart_steps`, …) |
| `PgBackend` | Facade — owns both `PostgresStorage` and `PostgresTaskScheduler` |
| `Worker` | Drives the poll loop, dispatches to the internal `TaskRegistry` |
| `WorkerBuilder` | Builder for `Worker`; wires `DurableRegistry` into the internal `TaskRegistry` |
| `DurableRegistry` | User-facing registry — maps task-name strings to handler instances |
...
async fn main() -> anyhow::Result<()> {
    ...
    durable
        .start_for::<Onboarding>("run-1", "onboarding", &OnboardingData { /* ... */ })
        .await?;
...
<Aside type="tip">
In most applications `PgBackend` is created once and shared via `Arc` so both the web server (scheduling) and the worker (executing) use the same backend.
</Aside>
...
| `zart::schedule` | `fn schedule<S: ZartStep>(s: S) -> StepHandle<S::Output>` | Register a step for parallel execution without waiting. | [Parallel Steps](/docs/rust-api/parallel-steps/) |
| `zart::wait` | `async fn wait<T>(handles: Vec<StepHandle<T>>) -> Result<Vec<Result<T, _>>, _>` | Durably await all handles returned by `schedule`. | [Parallel Steps](/docs/rust-api/parallel-steps/) |
| `zart::sleep` | `async fn sleep(name: &str, duration: Duration) -> Result<(), StepError>` | Suspend execution for a fixed duration. `name` must be unique and stable. | [Durable Loops](/docs/rust-api/loops/) |
...
| `zart::wait_for_event` | `async fn wait_for_event<T>(name: &str, timeout: Option<Duration>) -> Result<T, StepError>` | Suspend until an external event is delivered. | [Wait for Event](/docs/rust-api/wait-for-event/) |
| `zart::capture` | `async fn capture<T, F>(name: &str, f: F) -> Result<T, StepError>` | Persist a pure synchronous value durably. | [Capture Variables](/docs/rust-api/capture-variables/) |
| `zart::now` | `async fn now(name: &str) -> Result<DateTime<Utc>, StepError>` | Persist the current UTC time durably. Shorthand for `capture(name, Utc::now)`. | [Capture Variables](/docs/rust-api/capture-variables/) |
...
1. **Schedule** — `DurableScheduler::start_for()` inserts a row into `zart_executions` with status `scheduled`.
2. **Claim** — Worker polls with `SELECT … FOR UPDATE SKIP LOCKED`. One worker owns one execution at a time.
3. **Run** — `DurableExecution::run()` is called. Each step call checks `zart_steps` for an existing result.
...
- [DurableExecution Trait](/docs/rust-api/task-handler/) — full reference for the workflow trait
- [ZartStep Trait](/docs/rust-api/zart-step/) — defining durable steps
- [Capture Variables](/docs/rust-api/capture-variables/) — persist synchronous values durably
- [Durable Loops](/docs/rust-api/loops/) — iterate over collections durably
- [Parallel Steps](/docs/rust-api/parallel-steps/) — concurrent fan-out with `zart::schedule` and `zart::wait`
- [Wait for Event](/docs/rust-api/wait-for-event/) — external signals and human approvals
