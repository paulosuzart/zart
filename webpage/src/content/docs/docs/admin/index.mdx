...
let app = Router::new()
    .merge(zart_api::routes::api_router(AppState::new(durable)))
    .merge(admin_router(scheduler.clone()));
```

| Method | Path | Description |
|---|---|---|
| `POST` | `/zart/admin/v1/executions/:id/retry-step` | Retry a dead step |
| `POST` | `/zart/admin/v1/executions/:id/restart` | Full restart |
| `POST` | `/zart/admin/v1/executions/:id/rerun` | Selective rerun |
| `GET` | `/zart/admin/v1/executions/:id/runs` | List all runs |
| `POST` | `/zart/admin/v1/pause` | Create a pause rule |
| `GET` | `/zart/admin/v1/pause` | List pause rules |
| `POST` | `/zart/admin/v1/pause/:rule_id` | Soft-delete a pause rule |

### Example: retry-step via HTTP

```bash
curl -X POST http://localhost:8080/zart/admin/v1/executions/exec-001/retry-step \
...
```bash
curl -X POST http://localhost:8080/zart/admin/v1/pause \
  -H "Content-Type: application/json" \
