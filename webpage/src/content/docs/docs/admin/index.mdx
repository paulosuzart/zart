...
Preserved step results are **copied** from the old run into the new run immediately after the new run is created. When the body replays, it sees those steps as already completed and skips re-executing them — no extra code required in your handler.

- The copy happens right after `restart_run` returns; if it fails, the new run still exists and preserved steps fall back to re-execution (safe).
- Step *attempt* history from the old run is **not** copied — only the final completed result is visible in the new run.
- The copy is idempotent (`ON CONFLICT DO NOTHING`), so calling it twice is safe.
:::

It's your responsibility to decide which steps can be safely preserved. If a preserved step depends on the output of a rerun step, its cached result may be stale.
