...
```rust
let worker = zart::WorkerBuilder::new(storage.clone(), storage.task_scheduler())
...
All workflow state is persisted to PostgreSQL via the `zart` crate (`PostgresStorage`). No extra infrastructure — just a database you already have.
