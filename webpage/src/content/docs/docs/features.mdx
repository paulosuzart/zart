...
## 5. Recurring Durable Executions

Schedule durable workflows on a recurring basis using cron expressions or fixed-delay intervals. Each tick starts a new `DurableExecution` with full step checkpointing, retries, and observability.

**Overlap policies** control what happens when a new tick fires while a previous execution is still running:

| Policy | Behavior | Use Case |
|---|---|---|
| `SkipIfRunning` | Silently skips the new tick | Nightly reports, ETL pipelines |
| `CancelAndRestart` | Cancels running execution, starts fresh | Config refresh, cache warming |
| `AlwaysStart` | Unconditionally starts new execution | Independent audit windows |
...
See [Recurring Durable Executions](/docs/concepts/recurring-durable) for full documentation.
