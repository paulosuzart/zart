...
```toml
# Cargo.toml
zart = { version = "0.2", features = ["metrics"] }

# If you also use zart-api, enable its matching feature:
zart-api = { version = "0.2", features = ["metrics"] }
```
...
async fn health(State(worker): State<Arc<Worker>>) -> impl IntoResponse {
    // Worker exposes stop/cancellation signals for health checks.
    // Add application-specific health logic as needed.
    if worker.cancellation_token().is_cancelled() {
