...
:::note[Planned]
The Zart Java SDK is planned for a future release. This page describes the intended design. If you're interested in contributing or being an early tester, open an issue on [GitHub](https://github.com/paulosuzart/zart).
:::
...
public class OnboardingTask implements DurableExecution<OnboardingData, Void> {
    ...
    public Void run(TaskContext ctx, OnboardingData data) throws TaskException {
        ...
        ctx.executeStep(new SendWelcomeStep(data.getEmail()));

        // Step with retry policy
        String customerId = ctx.executeStep(
            new CreateCustomerStep(data.getEmail())
        );
...
class CheckoutTask : SuspendingDurableExecution<CheckoutData, Receipt> {
    override suspend fun run(ctx: TaskContext, data: CheckoutData): Receipt {
        val charge = ctx.executeStep(ChargeStep(data.card, data.total))
