...
impl DurableExecution for OnboardingTask {
        ...
        -> Result<(), TaskError>
    {
        ...
        ctx.execute_step(SendEmailStep { email: &data.email }).await?;

        // Retry up to 3x with exponential backoff if billing fails
        ctx.execute_step(SetupBillingStep { email: &data.email }).await?;
...
- [DurableExecution Trait](/rust-api/task-handler/) — full reference for `ctx.execute_step()`, retries, sleeps, and events
- [Macros](/rust-api/macros/) — the `#[zart_durable]` and `#[zart_step]` proc-macro layer for less boilerplate
