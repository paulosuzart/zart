...
// 1. Define steps as plain async functions — no ctx threading
...
async fn onboarding(data: OnboardingData) -> Result<(), TaskError> {
    ...
    setup_billing(&data.email).await?;

    Ok(())
}

// 3. Register and run a worker
let mut registry = TaskRegistry::new();
registry.register("onboarding", Onboarding);

Worker::new(scheduler, registry, WorkerConfig::default()).run().await;

// 4. Schedule an execution (idempotent — safe to call twice with the same ID)
...
- [DurableExecution Trait](/rust-api/task-handler/) — full reference for steps, retries, sleeps, and events
- [Macros](/rust-api/macros/) — the `#[zart_durable]` and `#[zart_step]` proc-macro layer
- [Parallel Steps](/rust-api/parallel-steps/) — run steps concurrently with `zart::wait`
