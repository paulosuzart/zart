...
Or use the handler type to infer both input and output:

```rust
let output = durable
    .start_and_wait_for::<Onboarding>("signup-user-42", &input, Duration::from_secs(60))
    .await?;
// output: () — inferred from Onboarding::Output
```
...
See the full [Examples](/examples/brewery-finder/) section for detailed walkthroughs.

## Next Steps

- [**Durable Execution**](/concepts/durable-execution/) — the core concept, lifecycle, and idempotency
- [**Steps**](/concepts/steps/) — defining steps, retries, sleeps, events, and durable values
- [**Flow Control**](/concepts/flow-control/) — parallel steps and durable loops
