...
pub trait ZartStep {
    ...
    /// Unique name for this step (used in database tracking).
...
The framework reads `step_name()` for tracking, applies `retry_config()` if set, and calls `run()` to execute the step logic. Results are persisted to the database automatically.
...
impl ZartStep for ProcessApprovedStep {
    type Output = String;
    fn step_name(&self) -> &'static str { "process-approved" }
    async fn run(&self, _ctx: StepContext) -> Result<Self::Output, StepError> {
...
impl DurableExecution for ApprovalTask {
    ...
    async fn run(
        ...
        // Wait for manager approval (up to 2 hours)
