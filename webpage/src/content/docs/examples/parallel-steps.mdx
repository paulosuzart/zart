...
**Features demonstrated:** `#[zart_step]`, `ctx.schedule_step()`, `ctx.wait_all()`, structured output, `DurableExecution` trait.
...
**`ctx.schedule_step(step)`** — registers the step for execution without waiting. Returns a handle. The step is enqueued in the scheduler and may run on any available worker.
