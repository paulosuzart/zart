...
This example processes a batch of reports. It fetches the list inside a step for stable replay, then iterates over it with two uniqueness mechanisms: the `{index}` template and `.with_id()`.

**Features demonstrated:** `#[zart_step("name-{field}")]`, `.with_id()`, `ctx.execute_step()` in a loop, fetch-inside-step pattern, `DurableExecution` trait.
...
/// Static step name — callers must supply `.with_id()` when calling in a loop.
...
impl DurableExecution for ReportBatchTask {
    ...
    async fn run(
        ...
        // .with_id() — override the static name at the call site.
...
durable
    .start_typed(&execution_id, "report-batch", &BatchInput {
...
**`.with_id()`** — when the unique key isn't part of the step struct (or the step is reused under different naming schemes), override the name at the call site: `.with_id(format!("notify-stakeholder-{i}"))`.

See [Durable Loops](/rust-api/loops/) for full documentation on both mechanisms and guidance on when to use each.
