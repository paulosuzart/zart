...
**Features demonstrated:** Manual `DurableExecution` trait with dependency injection, radkit `LlmFunction<T>` structured outputs, `ctx.step()`, `ctx.step_with_retry()`, mixed AI + API workflows.
