...
**Features demonstrated:** `#[zart_durable]`, `#[zart_step]`, `ctx.execute_step()`, sequential steps, structured output.
...
The durable handler composes steps using `ctx.execute_step()`. The body is pure orchestration — no closures, no manual `.clone()` capture.
...
**`#[zart_step]` macro** — turns an async function into a step struct that implements `ZartStep`. The step name and retry config are specified as attributes. Each parameter (except `ctx: StepContext`) becomes a field on the generated struct.
...
**`ctx.execute_step()`** — executes a step struct returned by the generated builder function. The framework reads `step_name()` and `retry_config()` from the `ZartStep` trait and applies them automatically.
