...
**Features demonstrated:** `TaskHandler` trait, `ctx.step()`, `ctx.wait_for_event()`, `durable.offer_event()`.
...
This example uses the `TaskHandler` trait directly (without the `#[zart_durable]` macro) to show the underlying mechanics.
...
**Use cases** — manager approval flows, interactive onboarding, integration with external review systems, human-in-the-loop AI agent chains.
