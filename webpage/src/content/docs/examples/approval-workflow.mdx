...
**Features demonstrated:** `#[zart_durable]`, `#[zart_step]`, `ctx.execute_step()`, `ctx.wait_for_event()`, `durable.offer_event()`.
...
The durable handler composes steps and uses `wait_for_event` to pause for the manager's decision.

```rust
#[zart_durable("approval-task", timeout = "5m")]
async fn approval_task(
    ctx: &mut TaskContext,
    data: ApprovalRequest,
) -> Result<ApprovalOutput, TaskError> {
    // Step 1: validate the request
    ctx.execute_step(validate_request(&data)).await?;
    ...
    if decision.approved {
        ctx.execute_step(process_approved(&data.resource, &data.requester_name)).await?;
    }
